/* =============================================
   My Portfolio v3.13.2 — Modals UI
   Desktop UI Overhaul: Centered modals with scale animation
...
function closeAllModals() {
  ...
  modals.forEach(m => { m.setAttribute('aria-hidden', 'true'); m.removeAttribute('aria-modal'); m.classList.add('closing'); });
...
function renderCategorySelector(selectedId, containerId) {
  return `<div class="cat-select" id="${containerId}" role="radiogroup" aria-labelledby="${containerId}Label">
    ${CATEGORIES.map(c => `<button class="cat-btn ${c.id === selectedId ? 'active' : ''}" data-cat="${escAttr(c.id)}" data-action="select-cat" role="radio" aria-checked="${c.id === selectedId ? 'true' : 'false'}">${c.icon} ${escHtml(c.label)}</button>`).join('')}</div>`;
}

function selectCat(btn) {
  const group = btn.closest('.cat-select');
  if (group) group.querySelectorAll('.cat-btn').forEach(b => { b.classList.remove('active'); b.setAttribute('aria-checked', 'false'); });
...
function renderIncomeCatSelector(selectedCat, labelId) {
  return `<div class="cat-select" role="radiogroup" aria-labelledby="${labelId}">
    ${INCOME_CATS.map(c => `<button class="cat-btn ${c.id === selectedCat ? 'active' : ''}" data-cat="${escAttr(c.id)}" data-action="select-inc-cat" role="radio" aria-checked="${c.id === selectedCat ? 'true' : 'false'}">${c.icon} ${escHtml(c.label)}</button>`).join('')}</div>`;
}
...
function _setupModalMainDelegation(container) {
  const handler = (e) => {
    ...
    else if (action === 'toggle-wallet-all') { const cb = target.querySelector('input[type="checkbox"]') || target; toggleWalletAll(cb.checked); }
...
function openAddAsset() {
  ...
  container.innerHTML = `<div class="modal-backdrop"></div><div class="modal-box"><div class="modal-header"><h3>자산 추가</h3><button class="modal-close" data-action="close-modal" data-modal="modalMain" aria-label="닫기">✕</button></div><div class="modal-body">
    ...
    <div class="form-row"><div class="form-group"><label for="txDate">날짜</label><input type="date" id="txDate" value="${today()}"></div><div class="form-group"><label for="txAccount">계좌</label><input type="text" id="txAccount" placeholder="선택사항" maxlength="50"></div></div>
  ...
  openModal('modalMain'); updateFormFields('국내주식'); _setupModalMainDelegation(container);
}
...
function doAddAsset() {
  ...
  const asset = addAsset({ name, category: cat, amount: price,
    stockCode: $('#assetCode')?.value.trim() || '', market: $('#assetMarket')?.value || '',
    coinId: $('#coinSelect')?.value || '', isUsdt: $('#isUsdt')?.checked || false,
    note: $('#assetNote')?.value.trim() || null, krxEtf: ETF_PREFIXES.some(p => name.toUpperCase().startsWith(p)),
    txns: price > 0 && qty > 0 ? [{ type: 'buy', price, qty, date: $('#txDate')?.value || today(), account: $('#txAccount')?.value.trim() || null, memo: null }] : [],
  });
  if (asset) { closeModal('modalMain'); showToast(`"${name}" 추가됨`, 'success'); render(); }
}
...
function openEditAsset(id) {
  ...
  const isCoin = asset.category === '코인', isCash = asset.category === '현금';
  _modalCleanup.removeAll();
  const container = $('#modalMain');
  container.innerHTML = `<div class="modal-backdrop"></div><div class="modal-box"><div class="modal-header"><h3>자산 수정</h3><button class="modal-close" data-action="close-modal" data-modal="modalMain" aria-label="닫기">✕</button></div><div class="modal-body">
    ...
    <div class="form-group"><label for="editPrice">현재 단가</label><input type="number" id="editPrice" value="${safeNum(asset.amount)}" min="0" step="any"></div>
  ...
  openModal('modalMain'); _setupModalMainDelegation(container);
}

function doEditAsset(id) {
  const cat = $$('#modalMain .cat-btn.active')[0]?.dataset?.cat;
  updateAsset(id, { name: $('#editName')?.value.trim() || '이름 없음', category: cat || '기타',
    stockCode: $('#assetCode')?.value.trim() || '', market: $('#assetMarket')?.value || '',
    coinId: $('#coinSelect')?.value || '', isUsdt: $('#isUsdt')?.checked || false,
    amount: safeNum($('#editPrice')?.value), note: $('#editNote')?.value.trim() || null });
  closeModal('modalMain'); showToast('수정되었습니다', 'success'); render();
}
...
function openAssetDetail(id) {
  ...
  container.innerHTML = `<div class="modal-backdrop"></div><div class="modal-box modal-large"><div class="modal-header"><h3>${escHtml(asset.name)}</h3><button class="modal-close" data-action="close-modal" data-modal="modalMain" aria-label="닫기">✕</button></div><div class="modal-body">
    ...
    <div class="txn-section" role="region" aria-label="거래 내역"><h4>거래 내역 (${asset.txns.length}건)</h4><div class="txn-list" role="list">
      ${asset.txns.length > 0 ? asset.txns.slice().reverse().map(t => `<div class="txn-item" role="listitem"><div class="txn-info"><span class="txn-type ${t.type}">${t.type === 'buy' ? '매수' : '매도'}</span><span>${escHtml(fmtDate(t.date))}</span>${t.account ? `<span class="txn-acct">${escHtml(t.account)}</span>` : ''}</div><div class="txn-values"><span>${escHtml(fmtPrice(t.price))} x ${escHtml(fmtNum(t.qty, t.qty % 1 !== 0 ? 4 : 0))}</span><span class="txn-total">${escHtml(fmtKRW(t.price * t.qty))}</span></div>${t.memo ? `<div class="txn-memo">${escHtml(t.memo)}</div>` : ''}<button class="btn-icon btn-danger txn-del" aria-label="거래 삭제" data-action="delete-txn" data-asset-id="${id}" data-txn-id="${t.id}">✕</button></div>`).join('') : '<div class="empty-state">거래 내역이 없습니다</div>'}
    </div></div></div></div>`;
  openModal('modalMain'); _setupModalMainDelegation(container);
}
...
function openTransaction(assetId, type = 'buy') {
  ...
  const calcTxTotal = () => { const p = safeNum($('#txnPrice')?.value), q = safeNum($('#txnQty')?.value); const el = $('#txnTotal'); if (el) el.textContent = fmtKRW(p * q); };
...
function _setTxCurrency(btn, currency) {
  $$('#modalSub .btn-group [role="radio"]').forEach(b => { b.classList.remove('active'); b.setAttribute('aria-checked', 'false'); });
  btn.classList.add('active'); btn.setAttribute('aria-checked', 'true');
  const input = $('#txCurrency'); if (input) input.value = currency;
  const label = $('#txCurrLabel'); if (label) label.textContent = currency;
}
...
async function doTransaction(assetId, type) {
  ...
  if (currency === 'USD') {
    try { const rate = await fetchExchangeRate(); price = Math.round(price * rate); }
    catch (e) { console.warn('doTransaction: exchange rate fetch failed', e); showToast('환율 조회 실패', 'error'); return; }
  }
  const asset = getAsset(assetId);
  if (!asset) return;
  if (type === 'sell') { const v = calcAssetValue(asset); if (qty > v.qty) { showToast(`보유 수량(${fmtNum(v.qty, 2)})을 초과합니다`, 'error'); return; } }
  const success = addTransactionWithPrice(assetId, { type, price, qty, date: $('#txnDate')?.value || today(), account: $('#txnAccount')?.value.trim() || null, memo: $('#txnMemo')?.value.trim() || null }, price);
  if (success) { closeModal('modalSub'); showToast(`${type === 'buy' ? '매수' : '매도'} 완료`, 'success'); openAssetDetail(assetId); }
}

function doDeleteTxn(assetId, txnId) {
  openConfirmModal('이 거래를 삭제하시겠습니까?', () => { deleteTransaction(assetId, txnId); openAssetDetail(assetId); showToast('거래 삭제됨'); });
}
...
function openPortfolioManager() {
  ...
  container.innerHTML = `<div class="modal-backdrop"></div><div class="modal-box"><div class="modal-header"><h3>포트폴리오 관리</h3><button class="modal-close" data-action="close-modal" data-modal="modalMain" aria-label="닫기">✕</button></div><div class="modal-body">
    <div class="pf-list" role="list" id="pfListContainer">
      ${meta.list.map(p => `<div class="pf-item ${p.id === activePortfolioId ? 'active' : ''}" role="listitem" data-pid="${escAttr(p.id)}"><span class="pf-name" data-action="switch" data-pid="${escAttr(p.id)}" role="button" tabindex="0" aria-current="${p.id === activePortfolioId ? 'true' : 'false'}">${escHtml(p.name)}</span><div class="pf-actions"><button class="btn-icon" data-action="rename" data-pid="${escAttr(p.id)}" aria-label="${escHtml(p.name)} 이름 변경">✎</button>${p.id !== 'default' ? `<button class="btn-icon btn-danger" data-action="delete" data-pid="${escAttr(p.id)}" aria-label="${escHtml(p.name)} 삭제">🗑</button>` : ''}</div></div>`).join('')}
    </div>
    ${meta.list.length < LIMITS.portfolios ? `<div class="form-group form-group-mt"><div class="form-row"><input type="text" id="newPfName" placeholder="새 포트폴리오 이름" maxlength="50" aria-label="새 포트폴리오 이름"><button class="btn-p" data-action="create-portfolio">생성</button></div></div>` : `<p class="text-muted">최대 ${LIMITS.portfolios}개 도달</p>`}
  </div></div>`;
  openModal('modalMain');
  ...
  if (pfContainer) {
    _modalCleanup.add(pfContainer, 'click', (e) => {
      ...
      const action = target.dataset.action, pid = target.dataset.pid; if (!pid) return;
      if (action === 'switch') doSwitchPortfolio(pid);
      else if (action === 'rename') { const m = loadPortfolioMeta(); const pf = m.list.find(p => p.id === pid); if (pf) openRenameModal(pid, pf.name); }
      else if (action === 'delete') { const m = loadPortfolioMeta(); const pf = m.list.find(p => p.id === pid); if (pf) doDeletePortfolio(pid, pf.name); }
    });
    _modalCleanup.add(pfContainer, 'keydown', (e) => { if (e.key === 'Enter' || e.key === ' ') { const target = e.target.closest('[data-action="switch"]'); if (target) { e.preventDefault(); doSwitchPortfolio(target.dataset.pid); } } });
  }
  _setupModalMainDelegation(container);
}

async function doSwitchPortfolio(pid) { if (pid === activePortfolioId) return; UIState.reset(); switchPortfolio(pid); await closeAllModals(); render(); showToast('포트폴리오 변경됨', 'success'); }
async function doCreatePortfolio() { const name = $('#newPfName')?.value.trim(); if (!name) { showToast('이름을 입력하세요', 'error'); return; } const id = createPortfolio(name); if (id) { UIState.reset(); switchPortfolio(id); await closeAllModals(); render(); showToast(`"${name}" 생성됨`, 'success'); } }

function openRenameModal(pid, currentName) {
  const container = $('#modalSub');
  container.innerHTML = `<div class="modal-backdrop"></div><div class="modal-box"><div class="modal-header"><h3>포트폴리오 이름 변경</h3><button class="modal-close" data-action="close-sub-modal" aria-label="닫기">✕</button></div><div class="modal-body"><div class="form-group"><label for="renamePfInput">새 이름</label><input type="text" id="renamePfInput" value="${escAttr(currentName)}" maxlength="50"></div><div class="modal-actions"><button class="btn-s" data-action="close-sub-modal">취소</button><button class="btn-p" data-action="do-rename">변경</button></div></div></div>`;
  openModal('modalSub');
  _setupModalSubDelegation(container, (action) => { if (action === 'do-rename') { const name = $('#renamePfInput')?.value.trim(); if (name) { renamePortfolio(pid, name); closeModal('modalSub'); openPortfolioManager(); } } });
}

function doDeletePortfolio(pid, name) { openConfirmModal(`"${name}" 포트폴리오를 삭제하시겠습니까?`, async () => { deletePortfolio(pid); loadData(); await closeAllModals(); render(); showToast('삭제됨'); }); }
...
async function doWalletScan() {
  ...
  if (!addr || !/^0x[0-9a-fA-F]{40}$/.test(addr)) { showToast('올바른 EVM 주소를 입력하세요 (0x + 40자)', 'error'); return; }
  saveWalletAddr(addr);
  const btn = $('#btnScan'); if (btn) btn.disabled = true;
  const progress = $('#walletProgress'); if (progress) { progress.classList.remove('hidden'); progress.classList.add('visible'); }
  const results = await scanWallet(addr, prog => { const pct = Math.round((prog.done / prog.total) * 100); const bar = $('#walletProgressBar'); const text = $('#walletProgressText'); if (bar) bar.style.width = pct + '%'; if (text) text.textContent = `${prog.done}/${prog.total} 체크 중... (${pct}%)`; });
  if (progress) { progress.classList.add('hidden'); progress.classList.remove('visible'); }
  if (btn) btn.disabled = false;
  const resultsDiv = $('#walletResults'); if (!resultsDiv) return;
  if (results.length === 0) { resultsDiv.innerHTML = '<div class="empty-state">발견된 자산이 없습니다</div>'; return; }
  resultsDiv.innerHTML = `<div class="wallet-results"><div class="form-group"><label><input type="checkbox" id="walletSelectAll" checked> 전체 선택</label></div>${results.map((r, i) => `<div class="wallet-item"><input type="checkbox" class="wallet-check" data-idx="${i}" checked aria-label="${escAttr(r.symbol)} 선택"><span class="wallet-symbol">${escHtml(r.symbol)}</span><span class="wallet-bal">${escHtml(fmtNum(r.balance, 4))}</span><span class="wallet-chains">${escHtml(r.chains.join(', '))}</span><span class="wallet-value">${escHtml(fmtKRW(r.valueKRW))}</span></div>`).join('')}<button class="btn-p btn-mt" data-action="import-wallet">선택 항목 가져오기</button></div>`;
  const selectAllCb = $('#walletSelectAll'); if (selectAllCb) _modalCleanup.add(selectAllCb, 'change', () => toggleWalletAll(selectAllCb.checked));
}

function toggleWalletAll(checked) { $$('.wallet-check').forEach(cb => cb.checked = checked); }

function doImportWallet() {
  const selected = $$('.wallet-check:checked').map(cb => walletScanResults[Number(cb.dataset.idx)]).filter(Boolean);
  if (selected.length === 0) { showToast('선택된 항목이 없습니다', 'error'); return; }
  const count = importWalletAssets(selected);
  closeModal('modalMain'); showToast(`${count}개 자산 가져오기 완료`, 'success'); render();
}
