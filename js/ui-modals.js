/* =============================================
   My Portfolio v3.12.0 — Modals UI
   Cycle 15: Full rebuild from scratch
   P2 FIX: ALL modal content functions route through unified _setupModalMainDelegation
           (NO individual container.onclick = on #modalMain/#modalSub)
   P1 FIX: All IDs from uid() are strings — no Number() wrapping anywhere
   ============================================= */
...
function openModal(id) {
  ...
  // Double rAF ensures focus happens after browser paint
...
function _restoreFocus() {
  ...
  if (el && typeof el.focus === 'function' && document.contains(el)) {
    try { el.focus(); } catch (e) {
      console.warn('_restoreFocus failed:', e);
    }
  }
}
    ...
    setTimeout(() => {
      ...
      if (firstFocus && typeof firstFocus.focus === 'function' && document.contains(firstFocus)) {
        try { firstFocus.focus(); } catch (e) {
          console.warn('closeAllModals restoreFocus failed:', e);
        }
      }
      resolve();
    }, MODAL_ANIM_MS);
...
function openConfirmModal(msg, onConfirm) {
  ...
  container.innerHTML = `
    ...
    <div class="modal-box" role="alertdialog" aria-label="확인" aria-describedby="confirmMsg">
      <div class="modal-body">
        <p id="confirmMsg">${escHtml(msg)}</p>
        <div class="modal-actions">
          <button class="btn-s" data-action="close-confirm">취소</button>
          <button class="btn-p btn-danger" data-action="confirm-ok">확인</button>
        </div>
      </div>
    </div>
  `;
...
// ── Shared: Asset Category Selector (DRY) ──
function renderCategorySelector(selectedId, containerId) {
  return `
    <div class="cat-select" id="${containerId}" role="radiogroup" aria-labelledby="${containerId}Label">
      ${CATEGORIES.map(c => `
        <button class="cat-btn ${c.id === selectedId ? 'active' : ''}"
          data-cat="${escAttr(c.id)}" data-action="select-cat" role="radio"
          aria-checked="${c.id === selectedId ? 'true' : 'false'}">
...
// ── Shared: Income Category Selector (DRY) ──
function renderIncomeCatSelector(selectedCat, labelId) {
  return `
    <div class="cat-select" role="radiogroup" aria-labelledby="${labelId}">
      ${INCOME_CATS.map(c => `
        <button class="cat-btn ${c.id === selectedCat ? 'active' : ''}" data-cat="${escAttr(c.id)}"
          data-action="select-inc-cat" role="radio"
          aria-checked="${c.id === selectedCat ? 'true' : 'false'}">
          ${c.icon} ${escHtml(c.label)}
        </button>
      `).join('')}
    </div>
...
// ── P2 FIX: Unified modal main delegation via addEventListener ──
// Uses addEventListener through scoped Cleanup — never overwrites .onclick
function _setupModalMainDelegation(container) {
  const handler = (e) => {
    ...
    if (action === 'close-modal') {
      ...
      selectCat(target);
    } else if (action === 'select-inc-cat') {
      selectIncCat(target);
    } else if (action === 'do-add-asset') {
      doAddAsset();
    } else if (action === 'do-edit-asset') {
      // P1 FIX: ID from dataset is a string — keep as string
      doEditAsset(target.dataset.id);
    } else if (action === 'open-transaction') {
      openTransaction(target.dataset.id, target.dataset.type || 'buy');
    } else if (action === 'edit-asset-from-detail') {
      openEditAsset(target.dataset.id);
    } else if (action === 'delete-txn') {
      e.stopPropagation();
      doDeleteTxn(target.dataset.assetId, target.dataset.txnId);
    } else if (action === 'create-portfolio') {
      doCreatePortfolio();
    } else if (action === 'wallet-scan') {
      doWalletScan();
    ...
    } else if (action === 'do-add-income') {
      doAddIncome();
    } else if (action === 'do-edit-income') {
      // P1 FIX: ID from dataset is a string from uid() — keep as string, no Number()
      doEditIncome(target.dataset.id);
    }
  };
  _modalCleanup.add(container, 'click', handler);
}

// ── Unified sub-modal delegation ──
function _setupModalSubDelegation(container, extraHandler) {
  const handler = (e) => {
    ...
    if (action === 'close-sub-modal') {
      closeModal('modalSub');
    } else if (extraHandler) {
      extraHandler(action, target, e);
    }
  };
  _modalCleanup.add(container, 'click', handler);
}
...
function updateFormFields(cat) {
  ...
  const isCoin = cat === '코인';
  const isCash = cat === '현금';
  const stockF = $('#stockFields');
  const coinF = $('#coinField');
  const usdtF = $('#usdtField');
...
function openAddAsset() {
    ...
    <div class="modal-box">
      ...
      <div class="modal-body">
        <div class="form-group">
          <label id="catSelectLabel">카테고리</label>
          ${renderCategorySelector('국내주식', 'catSelect')}
        </div>

        <div class="form-group">
          <label for="assetName">자산명 *</label>
          <input type="text" id="assetName" placeholder="예: 삼성전자, QQQ, BTC" maxlength="100" required>
        </div>

        <div class="form-row" id="stockFields">
          <div class="form-group">
            <label for="assetCode">종목코드</label>
            <input type="text" id="assetCode" placeholder="예: 005930" maxlength="20">
          </div>
          <div class="form-group">
            <label for="assetMarket">시장</label>
        ...
        <div class="form-group hidden" id="coinField">
          <label for="coinSelect">코인 ID (CoinGecko)</label>
          <select id="coinSelect">
            <option value="">선택하세요</option>
            ${Object.entries(COIN_IDS).map(([sym, id]) =>
              `<option value="${escAttr(id)}">${escHtml(sym)} (${escHtml(id)})</option>`
            ).join('')}
          </select>
        </div>

        <div class="form-group hidden" id="usdtField">
          <label><input type="checkbox" id="isUsdt"> USDT (자동 환율 업데이트)</label>
        </div>

        <div class="form-group">
          <label for="assetNote">메모</label>
          <input type="text" id="assetNote" placeholder="선택사항" maxlength="500">
        </div>
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <input type="number" id="txPrice" placeholder="0" min="0" step="any">
          </div>
          <div class="form-group">
            <label for="txQty">수량</label>
            <input type="number" id="txQty" placeholder="0" min="0" step="any">
          </div>
        </div>
        <div class="form-row">
          <div class="form-group">
          ...
          <div class="form-group">
            <label for="txAccount">계좌</label>
            <input type="text" id="txAccount" placeholder="선택사항" maxlength="50">
          </div>
        </div>

        <div class="modal-actions">
          <button class="btn-s" data-action="close-modal" data-modal="modalMain">취소</button>
          <button class="btn-p" data-action="do-add-asset">추가</button>
        </div>
      </div>
    </div>
  `;
  ...
  // P2 FIX: use unified delegation — NOT container.onclick =
...
function doAddAsset() {
  ...
  const price = safeNum($('#txPrice')?.value);
  const qty = safeNum($('#txQty')?.value);

  const asset = addAsset({
    name, category: cat, amount: price,
    ...
    txns: price > 0 && qty > 0 ? [{
      ...
      account: $('#txAccount')?.value.trim() || null,
      memo: null,
    }] : [],
  });
...
function openEditAsset(id) {
  ...
  container.innerHTML = `
    ...
    <div class="modal-box">
      ...
      <div class="modal-body">
        <div class="form-group">
          <label id="editCatSelectLabel">카테고리</label>
          ${renderCategorySelector(asset.category, 'editCatSelect')}
        </div>
        <div class="form-group">
          <label for="editName">자산명</label>
          <input type="text" id="editName" value="${escAttr(asset.name)}" maxlength="100">
        </div>
        <div class="form-row ${isStock ? '' : 'hidden'}" id="stockFields">
          <div class="form-group">
            <label for="assetCode">종목코드</label>
            <input type="text" id="assetCode" value="${escAttr(asset.stockCode)}" maxlength="20">
          </div>
          <div class="form-group">
            <label for="assetMarket">시장</label>
            <select id="assetMarket">
              ${['KOSPI','KOSDAQ','NYSE','NASDAQ',''].map(m =>
                `<option value="${escAttr(m)}" ${asset.market === m ? 'selected' : ''}>${m || '기타'}</option>`
              ).join('')}
            </select>
          </div>
        </div>
        <div class="form-group ${isCoin ? '' : 'hidden'}" id="coinField">
          <label for="coinSelect">코인 ID</label>
          <select id="coinSelect">
            <option value="">선택하세요</option>
            ${Object.entries(COIN_IDS).map(([sym, cid]) =>
              `<option value="${escAttr(cid)}" ${asset.coinId === cid ? 'selected' : ''}>${escHtml(sym)}</option>`
            ).join('')}
          </select>
        </div>
        <div class="form-group ${isCash ? '' : 'hidden'}" id="usdtField">
          <label><input type="checkbox" id="isUsdt" ${asset.isUsdt ? 'checked' : ''}> USDT</label>
        </div>
        <div class="form-group">
          <label for="editPrice">현재 단가</label>
          <input type="number" id="editPrice" value="${safeNum(asset.amount)}" min="0" step="any">
        </div>
        <div class="form-group">
          <label for="editNote">메모</label>
          <input type="text" id="editNote" value="${escAttr(asset.note || '')}" maxlength="500">
        </div>
        <div class="modal-actions">
          <button class="btn-s" data-action="close-modal" data-modal="modalMain">취소</button>
          <button class="btn-p" data-action="do-edit-asset" data-id="${id}">저장</button>
        </div>
      </div>
    </div>
  `;
  ...
  container.innerHTML = `
    ...
    <div class="modal-box modal-large">
      ...
      <div class="modal-body">
        ...
        <div class="txn-section" role="region" aria-label="거래 내역">
          <h4>거래 내역 (${asset.txns.length}건)</h4>
          <div class="txn-list" role="list">
            ${asset.txns.length > 0
              ? asset.txns.slice().reverse().map(t => `
...
function openTransaction(assetId, type = 'buy') {
  ...
  container.innerHTML = `
    ...
    <div class="modal-box">
      <div class="modal-header">
        <h3>${escHtml(asset.name)} — ${type === 'buy' ? '매수' : '매도'}</h3>
      ...
      <div class="modal-body">
        ${isForeign ? `
          <div class="form-group">
            <label>통화</label>
            <div class="btn-group" role="radiogroup" aria-label="통화 선택">
              <button class="btn-sm active" data-action="set-tx-currency" data-currency="KRW" role="radio" aria-checked="true">KRW</button>
              <button class="btn-sm" data-action="set-tx-currency" data-currency="USD" role="radio" aria-checked="false">USD</button>
            </div>
            <input type="hidden" id="txCurrency" value="KRW">
          </div>
        ` : ''}
        <div class="form-row">
          <div class="form-group">
            <label for="txnPrice">단가 ${isForeign ? '(<span id="txCurrLabel">KRW</span>)' : ''}</label>
            <input type="number" id="txnPrice" placeholder="0" min="0" step="any">
          </div>
          <div class="form-group">
            <label for="txnQty">수량</label>
            <input type="number" id="txnQty" placeholder="0" min="0" step="any">
          </div>
        </div>
        <div class="tx-total" aria-live="polite">총액: <span id="txnTotal">₩0</span></div>
        <div class="form-row">
          <div class="form-group">
            <label for="txnDate">날짜</label>
            <input type="date" id="txnDate" value="${today()}">
          </div>
          <div class="form-group">
            <label for="txnAccount">계좌</label>
            <input type="text" id="txnAccount" placeholder="선택사항" maxlength="50">
          </div>
        </div>
        <div class="form-group">
          <label for="txnMemo">메모</label>
          <input type="text" id="txnMemo" placeholder="선택사항" maxlength="200">
        </div>
        <div class="modal-actions">
          <button class="btn-s" data-action="close-sub-modal">취소</button>
          <button class="btn-p" data-action="do-transaction" data-asset-id="${assetId}" data-type="${type}">
            ${type === 'buy' ? '매수' : '매도'}
          </button>
        </div>
      </div>
  ...
  _setupModalSubDelegation(container, (action, target) => {
    if (action === 'set-tx-currency') {
      _setTxCurrency(target, target.dataset.currency);
    } else if (action === 'do-transaction') {
      doTransaction(target.dataset.assetId, target.dataset.type);
    }
  });

  const calcTxTotal = () => {
    const p = safeNum($('#txnPrice')?.value);
    const q = safeNum($('#txnQty')?.value);
    const totalEl = $('#txnTotal');
    if (totalEl) totalEl.textContent = fmtKRW(p * q);
  };
  const txnPriceEl = $('#txnPrice');
  const txnQtyEl = $('#txnQty');
    ...
    <div class="modal-box">
      ...
      <div class="modal-body">
        <div class="pf-list" role="list" id="pfListContainer">
          ${meta.list.map(p => `
            <div class="pf-item ${p.id === activePortfolioId ? 'active' : ''}" role="listitem" data-pid="${escAttr(p.id)}">
              <span class="pf-name" data-action="switch" data-pid="${escAttr(p.id)}"
                role="button" tabindex="0" aria-current="${p.id === activePortfolioId ? 'true' : 'false'}">${escHtml(p.name)}</span>
        ...
        ${meta.list.length < LIMITS.portfolios ? `
          <div class="form-group form-group-mt">
            <div class="form-row">
              <input type="text" id="newPfName" placeholder="새 포트폴리오 이름" maxlength="50" aria-label="새 포트폴리오 이름">
              <button class="btn-p" data-action="create-portfolio">생성</button>
            </div>
          </div>
  ...
  if (pfContainer) {
    const pfClickHandler = (e) => {
      ...
      if (action === 'switch') doSwitchPortfolio(pid);
        ...
        const pfMeta = loadPortfolioMeta();
        const pf = pfMeta.list.find(p => p.id === pid);
    ...
    _modalCleanup.add(pfContainer, 'click', pfClickHandler);

    const pfKeyHandler = (e) => {
      if (e.key === 'Enter' || e.key === ' ') {
        const target = e.target.closest('[data-action="switch"]');
        if (target) { e.preventDefault(); doSwitchPortfolio(target.dataset.pid); }
      }
    };
    _modalCleanup.add(pfContainer, 'keydown', pfKeyHandler);
  }
...
function openRenameModal(pid, currentName) {
  ...
  container.innerHTML = `
    ...
    <div class="modal-box">
      ...
      <div class="modal-body">
        <div class="form-group">
          <label for="renamePfInput">새 이름</label>
          <input type="text" id="renamePfInput" value="${escAttr(currentName)}" maxlength="50">
        </div>
        <div class="modal-actions">
          <button class="btn-s" data-action="close-sub-modal">취소</button>
          <button class="btn-p" data-action="do-rename">변경</button>
        </div>
      </div>
    </div>
  `;
  ...
  container.innerHTML = `
    ...
    <div class="modal-box modal-large">
      ...
      <div class="modal-body">
        ...
        <div class="form-row">
          <input type="text" id="walletAddr" value="${escAttr(addr)}" placeholder="0x..." class="flex-1"
            aria-label="EVM 지갑 주소" pattern="^0x[0-9a-fA-F]{40}$">
          <button class="btn-p" id="btnScan" data-action="wallet-scan">스캔</button>
        </div>
        <div id="walletProgress" class="hidden" role="progressbar" aria-valuemin="0" aria-valuemax="100">
          <div class="progress-bar"><div class="progress-fill" id="walletProgressBar"></div></div>
          <div class="progress-text" id="walletProgressText" aria-live="polite">스캔 중...</div>
        </div>
...
async function doWalletScan() {
  ...
  resultsDiv.innerHTML = `
    <div class="wallet-results">
      <div class="form-group">
        <label><input type="checkbox" id="walletSelectAll" checked> 전체 선택</label>
      </div>
      ${results.map((r, i) => `
  ...
  if (selectAllCb) {
    _modalCleanup.add(selectAllCb, 'change', () => toggleWalletAll(selectAllCb.checked));
  }
}
...
function doImportWallet() {
  const selected = $$('.wallet-check:checked').map(cb =>
    walletScanResults[Number(cb.dataset.idx)]
  ).filter(Boolean);
